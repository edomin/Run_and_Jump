...
void ScreenInit(int w, int h, const char *title, bool fullscreen)
{
    ...
    SDL_version sdlver;
    /* Ќаходим все доступные режимы диспле€ */
    ...
    if (!fullscreen)
        Screen.window = SDL_CreateWindow(title, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, w, h, SDL_WINDOW_OPENGL);
    else
        Screen.window = SDL_CreateWindow(title, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, w, h, SDL_WINDOW_OPENGL | SDL_WINDOW_FULLSCREEN);

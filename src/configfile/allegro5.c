...
void ConfigfileSaveConfig(char *filename)
{
    if (Configfile.config != NULL)
    {
        al_save_config_file((const char *)filename,
                            (const ALLEGRO_CONFIG *)Configfile.config);
    }
}
...
void ConfigfileSetKeyValue(char *sectionName, char *key, char *value)
{
    if (Configfile.config != NULL)
    {
        al_set_config_value(Configfile.config, (const char *)sectionName,
...
char *ConfigfileGetKeyValue(char *sectionName, char *key)
{
    if (Configfile.config != NULL)
    {
        return (char *)al_get_config_value((const ALLEGRO_CONFIG *)Configfile.config,
